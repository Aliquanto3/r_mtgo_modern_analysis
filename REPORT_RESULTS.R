...
HistShare=1.2
metagame_box_plot(df,"Players",Beginning,End,EventType,HistShare)
...
HistShare=2
metagame_box_plot(df,"Matches",Beginning,End,EventType,HistShare)
...
#                           End,'_DF_Archetypes_Presence.csv',sep=''), 
#           row.names = TRUE)
###################################################################
...
#           paste('Results_as_CSV/',Beginning,'-',End,
#                 '_DF_Archetypes_Winrates.csv',sep=''), row.names = TRUE)
###################################################################
...
#           paste('Results_as_CSV/',Beginning,'-',End,
#                 '_DF_Archetypes_Metric_Comb.csv',sep=''), row.names = TRUE)
###################################################################
...
#           paste('Results_as_CSV/',Beginning,'-',End,
#                 '_DF_Archetypes_Lower_Winrate.csv',sep=''), row.names = TRUE)
###################################################################
...
regBestArch1=lm(WINRATE~RATIO_SORCERY+RATIO_INSTANT+RATIO_PLANESWALKER
                +RATIO_CREATURE+RATIO_LAND+RATIO_ARTIFACT+RATIO_ENCHANTMENT+
                  TOTAL_MD+CMC,data=df_best_arch)
...
fviz_eig (res.pca1)
fviz_pca_var(res.pca1, col.var = "contrib", gradient.cols = c("blue", "green", 
                                                              "red"))
